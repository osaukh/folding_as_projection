import torch
from utils import sharpness_utils as utils  # local
import copy
import math
    ...
    with torch.no_grad():
        ...
        if norm == 'l2':  # Project onto L2-ball of radius rho (* ||w|| if adaptive)
            def weighted_norm(delta_dict):
                return sum(
                    [((delta_dict[param] / (orig_param_dict[param].abs() if adaptive else 1)) ** 2).sum() for param in
                     delta_dict]) ** 0.5
...
def eval_average_sharpness(
    ...
    orig_param_dict = {param_name: p.clone() for param_name, p in
                       model.named_parameters()}  # {param: param.clone() for param in model.parameters()}
