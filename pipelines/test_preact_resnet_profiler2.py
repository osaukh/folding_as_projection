...
# -----------------------------------------------------------------------------
# Per-layer stats: params, FLOPs, activations
# -----------------------------------------------------------------------------
def _is_counted_module(m):
    return isinstance(m, (nn.Conv2d, nn.Linear))


def collect_layer_stats(model, device, input_size=(3, 32, 32)):
    """
...
def compare_layer_stats(stats_a, stats_b, name_a="fold", name_b="mag"):
    ...
    """
    print("\n" + "=" * 80)
    print(f"PER-LAYER COMPARISON: {name_a} vs {name_b}")
    print("=" * 80)
    ...
    header = (
        ...
        f"{'FLOPs_'+name_a:>14s}{'FLOPs_'+name_b:>14s}{'ΔF':>10s}"
        f"{'act_'+name_a:>12s}{'act_'+name_b:>12s}{'Δa':>10s}"
        f"{'nz_'+name_a:>12s}{'nz_'+name_b:>12s}{'Δnz':>10s}"
    )
    print(header)
    print("-" * len(header))
    ...
    for layer in all_layers:
        ...
        if sa is None or sb is None:
            print(f"{layer:40s}  MISSING IN ONE MODEL")
            continue
        ...
        print(
            ...
            f"{fa:14d}{fb:14d}{(fa-fb):10d}"
            f"{aa:12d}{ab:12d}{(aa-ab):10d}"
            f"{nza:12d}{nzb:12d}{(nza-nzb):10d}"
        )
    ...
    print(
        ...
        + f"{total_flops_a:14d}{total_flops_b:14d}{(total_flops_a-total_flops_b):10d}"
        + f"{total_act_a:12d}{total_act_b:12d}{(total_act_a-total_act_b):10d}"
        + f"{total_nz_a:12d}{total_nz_b:12d}{(total_nz_a-total_nz_b):10d}"
    )
...
def main():
    ...
    # Store per-layer stats for each method
    ...
    # -------------------------------------------------------------------------
    # Per-layer comparison between folding and magnitude pruning
    # -------------------------------------------------------------------------
    if "fold" in per_method_layer_stats and "mag" in per_method_layer_stats:
        ...
        print("\n[WARN] Missing per-layer stats for 'fold' or 'mag'; "
              "cannot perform per-layer comparison.")
