...
# ------------------------------------------------------------------------
# Make sure these two functions are available in your codebase.
# If they live elsewhere, just change this import accordingly.
...
def compute_sharpness_for_rhos(model, train_loader, loss_fn, rho_values, n_iters=10):
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate PreActResNet18 compression & APGD sharpness across ratios/checkpoints"
    )
    ...
    for i, ckpt_path in enumerate(ckpt_paths):
        ...
        for ratio in prune_ratios:
            ...
            log_line(ratio,"AfterRepair", Params=pruned_params, RESULT=f"TestAcc={acc_repair:.2f}")
            ...
            sharp_vals = compute_sharpness_for_rhos(
                model, train_loader, loss_fn,
                rho_values=rho_values, n_iters=args.sharp_iters
            )
